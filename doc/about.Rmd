...
Our data come from the [John Hopkins University dataset](https://github.com/CSSEGISandData/COVID-19), which is updated daily.  These data report the cumulative number of infections ($I_t$) and number of deaths ($D_t$) in each country.  The raw case numbers are simply a report of their raw numbers for the selected country at the current date.

### Current active cases

Active cases ($A_t$) are simply $A_t = I_t-D_t-R_t$ where $R_t$ is cumulative recoveries at time $t$.  As of 23 March, however, JHU no longer report recoveries in the US and Canada at State/Province level.  As a consequence, for these countries. we have had to assume that active cases take 22 days to resolve.  Thus, $A_t = I_t-D_t-(I_{t-22}-D_{t-22})$.
